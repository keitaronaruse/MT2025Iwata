...
// const double d_u = 0.0025, u_min = -1.0 - d_u / 2.0, u_max = 1.0 + d_u / 2.0;
// //  Range of v-position [ v_min, v_max )
// const double d_v = 0.0025, v_min = -1.0 - d_v / 2.0, v_max = 1.0 + d_v / 2.0;
...
// //  Data Set 2
// //  Range of u-position [ u_min, u_max )
// const double d_u = 0.0025, u_min = 1.800 - d_u / 2.0, u_max = 2.400 + d_u / 2.0;
// //  Range of v-position [ v_min, v_max )
...
int main( ) {
    ...
    while( !pri_que.empty( ) ) {
        ...
        for( auto w : W ) {
            ...
            //  Next state
            int u_id_next = u_id( u_next ), v_id_next = v_id( v_next ), q_id_next = q_id( q_next );
            if( t_curr + 1.0 < g_cost.at( u_id_next ).at( v_id_next ).at( q_id_next ) ) {
                ...
                prev.at( u_id_next ).at( v_id_next ).at( q_id_next ) = { u_id_curr, v_id_curr, q_id_curr };
                num_searched++;
            }
        }
    }
    ...
    std::vector< state > path_state = { state( u_id_curr, v_id_curr, q_id_curr ) };
    while( prev.at( u_id_curr ).at( v_id_curr ).at( q_id_curr ) != state{ -1, -1, -1 } ) {
        path_state.push_back( prev.at( u_id_curr ).at( v_id_curr ).at( q_id_curr ) );
